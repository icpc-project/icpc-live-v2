...
public class PollWidget extends Widget {
    ...
    protected void paintImpl(AbstractGraphics g, int screenWidth, int screenHeight) {
        ...
        for (int i = 0; i < total; i++) {
            drawTextInRect(g, options[i].getOption(), 0, y, optionWidth, plateHeight,
            ...
            drawTextInRect(g, "" + options[i].getVotes(), optionWidth, y,
                    ww, plateHeight,
    ...
    private void calcPoll() {
        ...
        if (pollToShow.getTeamOptions()) { // Then this seems to be the team poll
            Arrays.sort(options, (o1, o2) ->
                    o1.getVotes() == o2.getVotes() ? o1.getOption().compareTo(o2.getOption()) : o2.getVotes() - o1.getVotes()
            );
        }
        ...
        for (int i = 0; i < total; i++) {
            percent[i] = sum == 0 ? 0 : 1. * options[i].getVotes() / sum;
        }
    }
