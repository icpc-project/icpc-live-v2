...
public class TeamData extends CachedData {
    ...
    public void switchOverlaysOff() {
        ...
        if (mainScreenService.getPvpData().isVisible()) {
            mainScreenService.getPvpData().hide();
            turnOff = true;
        }
        if (turnOff) {
...
//        log.info("STOOOOOOOOOOOP IT!");
    ...
    public void setSleepTime(int sleepTime) {
        MainScreenService.getProperties().getTeamProperties().setSleepTime(sleepTime);
    ...
    public synchronized void update() {
        ...
        if (timestamp + MainScreenService.getProperties().getTeamProperties().getAutomatedShowTime() < System.currentTimeMillis()) {
            if (currentPosition + 1 < teamsToShow.length) {
                setInfo(automaticType, teamsToShow[++currentPosition], automaticStats);
            } else {
                if (timestamp + MainScreenService.getProperties().getTeamProperties().getAutomatedShowTime() +
